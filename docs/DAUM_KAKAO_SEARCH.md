...
| 다음 점유율(2025) | **약 3%, 국내 4위** (Bing이 근소 우위로 3위) | 단독 실효성 낮으나 **네이트가 다음 결과 그대로 노출** → 실효 청중은 점유율보다 큼 |
| 네이버 | 약 60~66% (압도적 1위) | 한국인 청중의 주력은 네이버(별도 차원, 이 문서 범위 밖) |
...
### 2-C. ⚠ 생성형 AI 검색 — '미도입' 전제 무효화 임박

과거 자료의 "다음은 자체 생성형 AI 검색을 아직 전면 도입하지 않았다"는 서술은 **STALE/진행형 변수**다. 업스테이지가 Solar LLM을 다음 검색엔진에 결합하는 AI 포털 고도화를 공식 추진 중([fnnews.com](https://www.fnnews.com/news/202605070819041556)). → 다음이 AI 답변형으로 전환하면 이 문서의 "한국인 청중 트랙"이 **AEO/GEO 트랙**([`./AEO_GEO_PLAYBOOK_2026.md`](./AEO_GEO_PLAYBOOK_2026.md))과 교차하므로, 재편 완료 시 GEO 관점 재평가.
...
### 2-E. 점유율 (⚠ 구체 소수점 단정 금지)

- 다음: 2024 약 3.3~3.7% → 2025 약 3%(4위)로 하락. Bing이 다음을 제치고 3위로 올라선 **방향성은 확실**하나, Bing 구체 수치는 출처별 편차가 큼(예: 2025-06 웹검색 약 4.1% / 연평균 3%대). **특정 소수점(예: "Bing 3.12%")은 단정하지 말 것.**
- 네이버 약 60~66% 압도적 1위 유지.
- 서술 기준: **"다음 약 3%·4위, Bing이 근소 우위 3위, 네이버 60%대 1위"** 수준으로만.
...
> Daum 고객센터 FAQ [28967](https://cs.daum.net/faq/service/15/category/4118/detail/28967)(2026-07-01 대조) — "웹문서 검색결과에 나오게 하려면" **4단계**: ①robots에서 Daumoa 수집 허용 → ②응답 상태 정상(4XX/5XX·연결실패 없어야) → ③아래 12개 제한기준에 안 걸릴 것 → ④조건 충족 후 **고객센터 '문의하기'로 등록 요청**(수집 수개월). 등록·사이트맵 제출을 해도 아래에 걸리면 색인 제외될 수 있다.
