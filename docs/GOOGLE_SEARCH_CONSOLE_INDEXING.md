...
9. **삭제/오발행/민감(PII·잘못된 비자·잘못된 가격) 페이지**는 GSC 임시삭제(약 180일)로 즉시 숨기고, **동시에** 404/410 또는 noindex로 영구 처리. robots.txt 차단만으로 색인 제거를 시도하지 않는다. (§10)
...
15. **Vercel 프리뷰 도메인(`*.vercel.app`)·스테이징은 noindex + robots 차단** (프로덕션 canonical 희석 방지). (§0.1, §15)
...
- **Passome/Korvia/ACT(Next.js)**: `app/sitemap.ts`로 동적 생성. **Vercel 프리뷰 도메인은 절대 색인시키지 말 것**(noindex + robots) — 프로덕션 canonical 중복/희석 방지.
