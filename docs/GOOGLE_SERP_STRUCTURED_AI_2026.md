...
| A4 | 모든 사이트 홈에 `Organization` 1개: `name`·`url`·`logo`(정사각 고해상)·`sameAs`(공식 SNS)·`knowsAbout`·`founder`·`foundingDate`. 듀얼타입은 **사이트별 분기**: 컨설팅(Passome·필요시 Korvia)=`['Organization','ProfessionalService']`, ACT=`['LocalBusiness','EducationalOrganization']`(CATALOG §6.2 통일), Kimchi(통신/eSIM 리테일)=`Organization` 단독(또는 `Store` 계열) — 전 사이트 강제 아님. | 로고·지식패널·사이트명. §1.3 |
...
| 타입 | 상태 | 공식 시점 | 근거 |
|---|---|---|---|
| **HowTo** | 완전 폐지(모바일·데스크톱 마크업 무시) | 2023-08 모바일 제거 → 2023-09 데스크톱 완전 폐지 | Search Central Blog 2023/08 |
...
> ⚠ 아래 JSON-LD는 **읽기용 예시**다 — **복붙 원본·프로젝트별 확정 템플릿은 [`SCHEMA_JSONLD_CATALOG.md`](./SCHEMA_JSONLD_CATALOG.md)를 쓴다**(이 문서 예시와 CATALOG가 다르면 CATALOG 우선). Organization `@type`은 **사이트별 분기**(A4): 컨설팅(Passome·필요시 Korvia)만 `['Organization','ProfessionalService']`, ACT=`['LocalBusiness','EducationalOrganization']`, Kimchi(통신/eSIM 리테일)=`Organization` 단독(또는 `Store` 계열) — 아래 예시는 Korvia(컨설팅)라 듀얼타입을 쓴다.
...
```json
{
  ...
  "logo": "https://www.korvia.com/logo-square.png",
...
```
→ 로고·지식패널·사이트명 노출에 직접 반영(`logo`는 정사각·고해상 이미지). 엔티티 일관성(동일 명칭·로고·바이오)이 핵심.
...
```json
{
  ...
  "author": { "@type": "Person", "name": "Henry Oh",
              "url": "https://www.korvia.com/about/henry-oh" },
...
> ⚠ 이 표는 2026-07 기준 스냅샷 — 최신 코어/스팸 업데이트는 매 발행 시 **Search Status Dashboard**(status.search.google.com)로 재확인.
...
| 2026-05-21 ~ **2026-06-02** | 코어 (May 2026 Core) | KB 생성(2026-07) 직전 완료, 12일 롤아웃 (Google Search Status Dashboard 확인) |
...
- **⚠ 신규(2026-06-03): GSC 자체 실측 도입** — Google Search Console에 **생성형 AI 성능 리포트**가 추가돼, AI Overviews·AI Mode 등 Search(및 Discover)의 생성형 AI 기능에서 내 URL이 노출된 **impressions**를 볼 수 있다. 단 **impressions만 제공**(clicks·CTR·평균순위·쿼리 분해 없음)이고 페이지·국가·기기·날짜별로만 분해되며, 초기 단계적 롤아웃(영국 우선)이라 아직 전 사이트 대상은 아니다 → 위 3자 노출률 추정을 **자사 실측 impressions로 보완**. 상세=[`GOOGLE_SEARCH_CONSOLE_INDEXING.md`](./GOOGLE_SEARCH_CONSOLE_INDEXING.md). (출처: developers.google.com/search/blog/2026/06/gen-ai-performance-reports)
...
> **[한국어 라이브]** AI Mode는 2025-09-08 힌디어·인도네시아어·**일본어·한국어**·브라질 포르투갈어 5개 언어로 확장됐다(영어 실험 2025-03 이후 첫 언어 확장, 180개국+, 커스텀 Gemini 2.5 구동) → **Korvia·Kimchi의 한국어 콘텐츠도 AI Mode 인용 대상**이다. (출처: blog.google/products/search/ai-mode-expands-more-languages/, 2025-09-08)
