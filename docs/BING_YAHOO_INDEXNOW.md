...
6. **Google·Daum은 IndexNow 미참여** → Google 색인은 GSC/사이트맵, Daum은 자체 제출(다른 문서). IndexNow는 **Bing·Naver·Yandex·Seznam·Yep·Amazon** 계열 전파로만 취급(참여 목록 SSOT=§4.1). — §4.1
7. **한국인 대상 페이지도 IndexNow 커버.** Naver가 IndexNow 참여. ⚠ 단, 제네릭 엔드포인트의 Naver 실전파는 공식 미확인 → **한국 트래픽 핵심 페이지는 Naver 직접 엔드포인트 병행 핑 권장**. — §4.1
8. **모든 신규 페이지에 JSON-LD** + **페이지 실체에 맞는 사이트 기본 타입만**(Korvia=`Article`[저자·날짜 필수] / Kimchi=`Product`·`Offer` / ACT=`LocalBusiness`·`Course`·`Event` / Passome=`Organization`·`Service`·`Article`). 각 타입의 required 속성(날짜/이름/가격 등) 필수 — 누락 시 Bing이 무시. (빌드 헬퍼는 WEB_PLAYBOOK) — §5
   - ⚠ **`FAQPage`·`HowTo`는 "모든 신규 페이지" 기본에서 제외**(자동 적용 대상 아님). Google 리치결과 폐지(HowTo=2023-09 전면 폐지 [출처: https://developers.google.com/search/blog/2023/08/howto-faq-changes] · FAQ=2026-05-07 노출 중단 [출처: https://developers.google.com/search/updates])이므로 **Google 노출 목적으로 생성 금지**. (a) 페이지가 실제 Q&A/단계형이고 **동시에** (b) Naver·AI(Bing·Copilot) 파싱 목적일 때만 ⚠ 달고 사용하며, Google 리치결과 기대치=0으로 둔다. (상세는 §5)
...
13. `robots.txt`에서 **`bingbot`과 `OAI-SearchBot`을 차단하지 않는다**(AI 가시성). AI 크롤러 허용 목록의 owner(SSOT)는 [`./ROBOTS_SITEMAP_RSS_TEMPLATES.md`](./ROBOTS_SITEMAP_RSS_TEMPLATES.md) §A. — §6
...
### 4.1 참여 검색엔진 — **확정 목록 [V8 · 이 문서가 SSOT]** (2026-07-02 확인 — `indexnow.org/faq`)

> **이 표가 IndexNow 참여 엔진 목록의 SSOT(owner)다.** `MULTI_ENGINE_MASTER`·`ROBOTS_SITEMAP_RSS_TEMPLATES`·`CONTENT_AUTORULES §9.2` 등 타 문서는 이 값을 재기입하지 말고 여기로 링크한다.

| 엔진 | IndexNow 엔드포인트(호스트) | 비고 |
|---|---|---|
| **Bing** (Microsoft) | `www.bing.com/indexnow` | Copilot/Yahoo/DuckDuckGo 백엔드 |
| **Naver** | `searchadvisor.naver.com/indexnow` | 🇰🇷 2023-07 지원 시작 — **한국인 청중 핵심** |
| **Yandex** | `yandex.com/indexnow` | |
| **Seznam.cz** | `search.seznam.cz/indexnow` | 체코 |
| **Yep** | `indexnow.yep.com/indexnow` | |
| **Amazon** | `indexnow.amazonbot.amazon/indexnow` | 공식 FAQ 참여 목록 포함(2026-07-02 확인) |
| **Google** | ❌ **미참여** | 2021년부터 테스트만, 2026년 현재 미채택. GSC/사이트맵/(제한적) Indexing API로 별도 |
| **Daum (Kakao)** | ❌ **미참여** | IndexNow 참여 목록에 없음 — Daum은 `webmaster.daum.net` 자체 제출로 별도([`./DAUM_KAKAO_SEARCH.md`](./DAUM_KAKAO_SEARCH.md)) |

> ⚠ **Internet Archive**(`web-static.archive.org/indexnow`)는 과거 `searchengines.json` 리스팅에 등장했으나 **2026-07-02 공식 FAQ 참여 목록에는 없다** → 확정 목록에서 제외(핑해도 무해하나 SSOT 대상 아님).
> **프로토콜:** 한 참여 엔진의 `/indexnow`로 제출한 URL은 다른 모든 참여 엔진과 자동 공유된다("submitted URLs will be automatically shared with all other participating search engines"; 검증 후 약 10초 내 전파, 배치당 최대 10,000 URL). (출처: https://www.indexnow.org/faq — 2026-07-02 확인)
...
3. **YMYL 사이트(Korvia=비자/취업, Kimchi=통신/금융)** 는 **문장 단위 검증가능성 + 출처·저자·발행/수정 날짜**를 강화해야 grounding 대상이 됨. 검증가능성 신호(숫자·명명 출처 인용문·출처)를 **문장 단위로 박는다**(Princeton GEO 상위 레버, 최대 ~40% 향상, ⚠2024 컷오프 — 개별 %를 특정 레버 하나에 단정 금지). 레버별 수치·상세는 SSOT [`./GEO_AEO_AI_ENGINES_2026.md`](./GEO_AEO_AI_ENGINES_2026.md) §B/§Δ 참조.
...
Cloudflare를 쓰는 사이트는 **Crawler Hints** 기능으로 **CDN 레벨에서 IndexNow를 자동 통합**할 수 있다(Next.js/WordPress 코드 변경 없이 엣지에서 핑). 정확한 위치: **Cloudflare 대시보드 > Caching > Configuration > Crawler Hints** 토글 ON(켜면 콘텐츠 변경을 감지해 IndexNow로 자동 제출). 4개 사이트가 Cloudflare 뒤에 있으면(Korvia IRS 스택은 Cloudflare Tunnel/R2 사용) 이 옵션을 검토 — 단 **변경/삭제분만 전송**되는지(전체 재핑 금지 원칙 유지)를 대시보드 설정에서 확인. ⚠ 대시보드 메뉴 구성은 개편될 수 있으니 실화면에서 경로 재확인.
...
7. **ChatGPT는 Bing에만 의존하지 말 것**: `robots.txt`에서 `OAI-SearchBot` 허용(SSOT=[`./ROBOTS_SITEMAP_RSS_TEMPLATES.md`](./ROBOTS_SITEMAP_RSS_TEMPLATES.md) §A) + 콘텐츠 품질/스키마로 대응. (§6)
...
> **분업 재확인:** AI 크롤러 robots.txt 허용 목록(SSOT owner) = [`./ROBOTS_SITEMAP_RSS_TEMPLATES.md`](./ROBOTS_SITEMAP_RSS_TEMPLATES.md) §A · JSON-LD 빌드 헬퍼 = WEB_PLAYBOOK §2 / [`./NEW_PAGE_CHECKLIST.md`](./NEW_PAGE_CHECKLIST.md) · GEO 전략/플랫폼별 인용 = [`./AEO_GEO_PLAYBOOK_2026.md`](./AEO_GEO_PLAYBOOK_2026.md) · llms.txt = AEO_GEO_PLAYBOOK §4 / WEB_PLAYBOOK §1.3. 이 문서는 그 위에 **Bing 계열 + IndexNow**만 얹는다.
