...
### 기술·크롤러
9. **robots.txt에 검색·인용 봇을 명시 Allow**(OAI-SearchBot, ChatGPT-User, PerplexityBot, Claude-SearchBot, Claude-User, DuckAssistBot, Applebot, Googlebot, Bingbot, Yeti). **절대 Disallow하거나 `User-agent:* Disallow:/` 범위에 포함하지 말 것** — 차단=답변에서 소멸.
...
| 통계 "+30%" | Princeton 원논문은 **통계 +41%**(상대). 순위: **통계 > 명명 출처/인용문(+28~40%) > 출처 추가**. 단 절대 %는 2024 컷오프. | KDD 2024 |
...
| 레버 | 효과(상대) |
|---|---|
| **통계(statistics) 추가** | **+41%** (최대 레버) |
| **명명된 출처 직접 인용(quotation)** | **+28~40%** |
| **출처(citations) 추가** | SERP 5위 사이트에 **+115.1%** 상대 가시성 |

유창성·전문용어 개선도 유효. **키워드 스터핑은 역효과.**

### B.2 2026 재현·확장
- **GEO-16 (arXiv 2509.10762, 2025-09)**: 16개 GEO 필러로 재측정. 인용과 최강 상관 = **메타데이터&신선도 r=0.68 > 시맨틱HTML r=0.65 > 구조화데이터 r=0.63**. `GEO≥0.70 & 필러≥12 → 교차엔진 인용률 78%, 고-GEO 오즈비 4.2배`. 엔진별 인용률 Brave 78% / Google AIO 72% / Perplexity 45%.
...
- **IP+UA 이중검증**: 스푸핑 방지 위해 WAF에서 각 사 공개 IP JSON으로 대조. 특히 `Perplexity-User`/`ChatGPT-User`는 UA 위조 흔함.
...
### E.2 서버 로그 / Cloudflare — AI봇 실측
- **서버 액세스 로그**에서 UA로 `GPTBot`·`OAI-SearchBot`·`ClaudeBot`·`PerplexityBot`·`Yeti`·`Bytespider` 요청수·상태코드·경로 집계 → "어떤 AI봇이 어디를 얼마나 크롤하나" 사실 확인.
- **Cloudflare `AI Crawl Control > Crawlers` 탭**: 봇별 요청수·allow/block·robots 위반 로그.
- ⚠ **GA4는 봇 트래픽을 필터링** → **AI봇 실측은 로그/CF가 정답**. GA4로 AI봇 재지 말 것.
...
| 도구 | 특징 | 가격(⚠ 급변, 도입 전 공식 페이지 재확인) |
|---|---|---|
| **Ahrefs Brand Radar** | **무료 티어 有**, Ahrefs 데이터 통합 | 무료 + 유료 $129/mo~ |
| **Otterly.AI** | 저가·프롬프트 추적·AIO 링크 추적 | $19~99/mo |
| **Peec AI** | 유럽발, 프롬프트 모니터·경쟁 벤치 | 중소($99~수백/mo) |
| **Profound** | 인용 DB·프롬프트 볼륨 최대·API. 누적 $155M, 2026-02 Series C $96M @ $1B, Fortune500 다수 | 엔터프라이즈 |
...
- ⚠ **Princeton GEO 절대 %**(+41%/+28~40%/+115%)는 2024 컷오프 — 상대 우선순위(통계>인용>출처)만 견고.
...
- GEO-16 (arXiv 2509.10762): https://arxiv.org/html/2509.10762v1
